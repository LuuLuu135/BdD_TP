...
INSERT INTO MAPA VALUES	('Sanctum Island'),
						('Western Expanse'),
						('Harmonia');					   						
...
INSERT INTO SESION VALUES ('2023-02-28', '15:45:00', '2023-02-28', '20:15:00', 'n0temueras'),
					   ('2023-03-12', '08:15', '2023-03-12', '10:30', 'Nikino'),
					   ('2023-05-06', '13:00', '2023-05-06', '18:45', 'XopiuX'),
					   ('2023-01-15', '10:30', '2023-01-15', '17:00', 'Fran41'),
					   ('2023-06-18', '09:45', '2023-06-18', '10:50', 'Evelayn18'),
					   ('2023-08-25', '12:00', '2023-08-25', '15:00', 'Fran#1223'),
					   ('2023-07-04', '17:30', '2023-07-04', '20:01', 'HalKryza'),
					   ('2023-02-28', '09:00', '2023-02-28', '12:00', 'DyingForFree'),
					   ('2023-10-05', '14:10', '2023-10-05', '17:09', 'MachiRulo'),
					   ('2023-11-23', '11:30', '2023-11-23', '13:20', 'Kryzah'),
					   ('2023-01-06', '04:45', '2023-01-06', '07:16', 'Rulo'),
					   ('2023-07-04', '11:50', '2023-07-04', '13:45', 'Alan Krzyzanowski'),
					   ('2023-03-10', '02:45', '2023-03-10', '04:14', 'luqiñas'),
					   ('2023-05-14', '11:15', '2023-05-14', '12:06', 'azyrk'),
					   ('2023-06-18', '16:30', '2023-06-18', '21:45', 'Sr Pates'),
					   ('2023-11-23', '12:30', '2023-11-23', '13:00', 'Tore'),
					   ('2023-10-03', '09:40', '2023-10-03', '13:01', 'Zeeb'),
					   ('2023-08-22', '10:10', '2023-08-22', '11:10', 'Melly'),
					   ('2023-09-12', '14:25', '2023-09-12', '19:35', 'Natokra'),
					   ('2023-03-10', '12:30', '2023-03-10', '14:08', 'RazMorningstar'),
					   ('2023-01-06', '10:45', '2023-01-06', '14:10', 'ConejoT1'),
					   ('2023-05-14', '23:30', '2023-05-15', '04:20', 'Senrielx'),
					   ('2023-02-28', '13:20', '2023-02-28', '20:19', 'LeonB'),
					   ('2023-05-06', '22:00', '2023-05-07', '00:45', 'XopiuX'),
					   ('2023-01-15', '08:00', '2023-01-15', '09:00', 'Fran41'),
					   ('2022-11-12', '15:20', '2022-11-12', '18:12', 'HypnosRequiem'),
					   ('2021-11-12', '15:20', '2021-11-12', '18:12', 'HypnosRequiem'),
					   ('2020-11-12', '15:20', '2020-11-12', '18:12', 'HypnosRequiem'),
					   ('2019-11-12', '15:20', '2019-11-12', '18:12', 'HypnosRequiem'),
					   ('2018-11-12', '15:20', '2018-11-12', '18:12', 'Fran41'),
					   ('2023-01-15', '20:00', '2023-01-16', '00:07', 'Fran41');
...
SELECT TOP 1 DATEPART(HOUR, Hora_in) AS Hora_Conexion, COUNT(*) AS Cantidad_Conexiones
FROM SESION
...
SELECT TOP 1 m.Nombre_Modalidad, COUNT(*) AS Cantidad
FROM PARTIDA p
JOIN MODALIDAD m ON p.ID_Modalidad = m.ID_Modalidad
GROUP BY p.ID_Modalidad, m.Nombre_Modalidad
ORDER BY COUNT(*) DESC;


-- Mapa más elegido:

SELECT TOP 1 mp.Nombre_Mapa, COUNT(*) AS Cantidad
FROM PARTIDA p
JOIN MAPA mp ON p.ID_Mapa = mp.ID_Mapa
GROUP BY p.ID_Mapa, mp.Nombre_Mapa
ORDER BY COUNT(*) DESC;
...
SELECT TOP 1 J.Nickname, H.Nombre_Heroe, COUNT(*) AS Cantidad_Victorias
FROM JUGADOR J
JOIN SESION S ON J.Nickname = S.Nickname
JOIN PARTIDA_SESION PS ON S.ID_Sesion = PS.ID_Sesion
JOIN HEROE H ON PS.ID_Heroe = H.ID_Heroe
WHERE PS.ganadores = 1
GROUP BY J.Nickname, H.Nombre_Heroe
ORDER BY COUNT(*) DESC;
...
-- Partida más larga con sus 10 jugadores:
...
SELECT MODALIDAD.Nombre_Modalidad, AVG(DATEDIFF(minute, '00:00', PARTIDA.Duracion)) AS DuracionPromedio
FROM PARTIDA
JOIN MODALIDAD ON PARTIDA.ID_Modalidad = MODALIDAD.ID_Modalidad
GROUP BY MODALIDAD.Nombre_Modalidad ;
...
WHERE HABILIDADES.Daño = ( SELECT MAX(HABILIDADES.Daño)
	...
	--OBTENER USUARIOS QUE NO INGRESARON EN LOS ULTIMOS 6 MESES:


	DECLARE @fecha_limite DATE = DATEADD(MONTH,-6,GETDATE());
	...
	SELECT * FROM JUGADOR
		WHERE Nickname IN (
			...
			FROM JUGADOR j
			LEFT JOIN (
				SELECT Nickname, MAX(Fecha_eg) AS UltimaFechaEgreso
				FROM SESION
				GROUP BY Nickname
			) s ON j.Nickname = s.Nickname
			WHERE s.Nickname IS NULL OR s.UltimaFechaEgreso < @Fecha_limite
		)
...
CREATE PROCEDURE ActualizarGanadasPerdidas
    @Nickname varchar(30),
    @PartidasGanadas int OUTPUT,
    @PartidasPerdidas int OUTPUT
AS
BEGIN

    SET @PartidasGanadas = 0;
    SET @PartidasPerdidas = 0;


	--AGREGAR UNA VICTORIA:

	DECLARE @partida int
	SELECT @partida = MAX(ID_Partida) + 1 FROM PARTIDA ;
	INSERT INTO PARTIDA VALUES (@partida,NULL,NULL,NULL,NULL);
	DECLARE @sesion int
	SELECT @sesion = MAX(ID_Sesion) FROM SESION ;
	INSERT INTO SESION VALUES (NULL,NULL,NULL,NULL,@Nickname);
	INSERT INTO PARTIDA_SESION VALUES (@partida,@sesion,NULL, 1)

    -- GANADAS Y PERDIDAS:
    SELECT @PartidasGanadas = COUNT(ganadores)
    FROM PARTIDA_SESION PS
    INNER JOIN SESION S ON PS.ID_Sesion = S.ID_Sesion
    INNER JOIN JUGADOR J ON S.Nickname = J.Nickname
    WHERE J.Nickname = @Nickname AND ganadores = 1;

    SELECT @PartidasPerdidas = COUNT(ganadores)
    FROM PARTIDA_SESION PS
    INNER JOIN SESION S ON PS.ID_Sesion = S.ID_Sesion
    INNER JOIN JUGADOR J ON S.Nickname = J.Nickname
    WHERE J.Nickname = @Nickname AND ganadores = 0;

END



DECLARE @Ganadas int, @Perdidas int;
EXEC ActualizarGanadasPerdidas 'HypnosRequiem', @Ganadas OUTPUT, @Perdidas OUTPUT;
SELECT @Ganadas AS PartidasGanadas, @Perdidas AS PartidasPerdidas;
...
GO
	...
	SELECT @Nickname AS Usuario, @PartidasGanadas AS Victorias,@PartidasPerdidas AS Derrotas
